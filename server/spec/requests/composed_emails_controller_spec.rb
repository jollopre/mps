...
RSpec.describe ComposedEmailsController do
  fixtures :users, :composed_emails
  fixtures :enquiries, :suppliers

  describe '#create' do
    it 'returns bad request when any parameter is missing' do
      post composed_emails_path, headers: auth_header(users(:user1))
    ...
    it 'returns bad request when the record is invalid' do
      post composed_emails_path, params: { composed_email: { foo: 'bar' } }, headers: auth_header(users(:user1))
      expect(response).to have_http_status(:bad_request)
    ...
    it 'returns not found when any association does not exist' do
      post composed_emails_path, params: { composed_email: { subject: 'foo', body: 'bar', enquiry_ids: [-1] }}, headers: auth_header(users(:user1))
      expect(response).to have_http_status(:not_found)
      expect(response_detail).to eq('Couldn\'t find Enquiry with \'id\'=[-1]')
      post composed_emails_path, params: { composed_email: { subject: 'foo', body: 'bar', supplier_ids: [-1] }}, headers: auth_header(users(:user1))
      expect(response).to have_http_status(:not_found)
    ...
    it 'returns success with location header set' do
      ...
      post composed_emails_path, params: { composed_email: { subject: 'foo', body: 'bar', enquiry_ids: enquiry_ids, supplier_ids: supplier_ids }}, headers: auth_header(users(:user1))
  ...
  describe '#update' do
    ...
    it 'returns bad request when any parameter is missing' do
      put composed_email_path(composed_email.id), headers: auth_header(users(:user1))
      expect(response).to have_http_status(:bad_request)
      expect(response_detail).to eq('param is missing or the value is empty: composed_email')
    end
    it 'returns bad request when the record is invalid' do
      put composed_email_path(composed_email.id), params: { composed_email: { subject: '', body: '' } }, headers: auth_header(users(:user1))
      expect(response).to have_http_status(:bad_request)
      expect(response_detail).to eq('Validation failed: Subject can\'t be blank, Body can\'t be blank')
    end
    it 'returns not found when any association does not exist' do
      put composed_email_path(composed_email.id), params: { composed_email: { subject: 'foo', body: 'bar', enquiry_ids: [-1] }}, headers: auth_header(users(:user1))
      expect(response).to have_http_status(:not_found)
      expect(response_detail).to eq('Couldn\'t find Enquiry with \'id\'=[-1]')
      put composed_email_path(composed_email.id), params: { composed_email: { subject: 'foo', body: 'bar', supplier_ids: [-1] }}, headers: auth_header(users(:user1))
      expect(response).to have_http_status(:not_found)
      expect(response_detail).to eq('Couldn\'t find Supplier with \'id\'=[-1]')
    end
  end
  ...
  describe '#show' do
    it 'returns not found when the id does not exist' do
      get composed_email_path(-1), headers: auth_header(users(:user1))
      expect(response).to have_http_status(:not_found)
    end
    it 'returns success with the record' do
      get composed_email_path(composed_emails(:composed_email1)), headers: auth_header(users(:user1))
  ...
  describe '#send_email' do
    it 'returns bad request when there are no enquiries associated' do
      ...
      post send_email_composed_email_path(composed_email), headers: auth_header(users(:user1))
      expect(response).to have_http_status(:bad_request)
    ...
    it 'returns success otherwise' do
      post send_email_composed_email_path(composed_emails(:composed_email1)), headers: auth_header(users(:user1))
      expect(response).to have_http_status(:created)
